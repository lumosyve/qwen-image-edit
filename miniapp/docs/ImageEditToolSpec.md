...
* 创建设置页面：`src/pages/settings/settings.vue`
* 在首页添加跳转到图片编辑页面和设置页面的入口
* 更新`pages.json`添加新页面路由

### 2. UI界面设计

* 图片上传区域：支持上传1-3张图片，使用自定义图片预览组件
...
* 图片上传处理：支持本地图片选择，使用uni.chooseImage API
...
✅ 图片编辑页面，支持上传1-3张图片
✅ Prompt输入区域，提供按模式分组的示例Prompt
✅ 参数配置区域，支持调整CFG值、推理步数、随机种子等参数
✅ 结果展示区域，支持预览和保存生成的图片
✅ 设置页面，允许用户配置SiliconFlow API Key
✅ 图片预览组件，用于统一处理图片的展示和操作
✅ 所有功能均已测试通过，代码结构清晰，符合uni-app开发规范